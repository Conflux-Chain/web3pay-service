...
// Modified with Original Author: https://github.com/ybbus/jsonrpc
package jsonrpc
...
var (
	ContextKeyCustomHeaders = ContextKey("Custom-Headers")
)

func NewContextWithCustomHeaders(ctx context.Context, customHeaders map[string]string) context.Context {
	return context.WithValue(ctx, ContextKeyCustomHeaders, customHeaders)
}

func CustomHeadersFromContext(ctx context.Context) map[string]string {
	v := ctx.Value(ContextKeyCustomHeaders)
...
func NewClientWithOpts(endpoint string, opts *RPCClientOpts) RPCClient {
	...
	if opts.Timeout != 0 {
		rpcClient.httpClient.Timeout = opts.Timeout
	}
...
func (client *rpcClient) Call(ctx context.Context, method string, params ...interface{}) (*RPCResponse, error) {
	customHeaders := CustomHeadersFromContext(ctx)
	...
	return client.doCall(ctx, request, customHeaders)
}
...
func (client *rpcClient) CallBatch(ctx context.Context, requests RPCRequests) (RPCResponses, error) {
	...
	return client.doBatchCall(ctx, requests, customHeaders)
}
...
func (client *rpcClient) newRequest(ctx context.Context, req interface{}, customHeaders map[string]string) (*http.Request, error) {
	...
	for _, headers := range []map[string]string{client.customHeaders, customHeaders} {
...
func (client *rpcClient) doCall(ctx context.Context, RPCRequest *RPCRequest, customHeaders map[string]string) (*RPCResponse, error) {

	httpRequest, err := client.newRequest(ctx, RPCRequest, customHeaders)
	if err != nil {
...
func (client *rpcClient) doBatchCall(ctx context.Context, rpcRequest []*RPCRequest, customHeaders map[string]string) ([]*RPCResponse, error) {
	httpRequest, err := client.newRequest(ctx, rpcRequest, customHeaders)
	if err != nil {
